# 🚀 Algorithms & Data Structures Toolkit

![C++](https://img.shields.io/badge/Language-C++-blue.svg)
![Status](https://img.shields.io/badge/Status-Active-brightgreen.svg)
![License](https://img.shields.io/badge/License-MIT-purple.svg)

Welcome to my comprehensive and efficient C++ algorithms toolkit! This repository contains a curated collection of classic and advanced data structures, graph algorithms, and competitive programming templates. Everything here is highly optimized and written in **C++** to help with problem-solving in top-tier competitive programming contests and coding interviews.

## 📂 Repository Contents

### 🌲 Graph Algorithms
- [`BFS.cpp`](./BFS.cpp) - Breadth-First Search
- [`DFS.cpp`](./DFS.cpp) - Depth-First Search
- [`Dijkstra.cpp`](./Dijkstra.cpp) - Dijkstra's Shortest Path Algorithm
- [`Is_Ancestor.cpp`](./Is_Ancestor.cpp) - Tree Lineage & Ancestor Checking

### 🛠 Data Structures
- [`Lazy_Segment_Tree(sum).cpp`](./Lazy_Segment_Tree(sum).cpp) - Segment Tree with Lazy Propagation (Range Sum)
- [`Sum_Segment_Tree.cpp`](./Sum_Segment_Tree.cpp) - Segment Tree (Point Update, Range Sum)
- [`Min_Sparse_Table.cpp`](./Min_Sparse_Table.cpp) - Sparse Table for Range Minimum Query (RMQ)
- [`Sum_Sparse_Table.cpp`](./Sum_Sparse_Table.cpp) - Sparse Table for Range Sum Query
- [`Red-Black_Tree.cpp`](./Red-Black_Tree.cpp) - Red-Black Tree Implementation

### 📐 Mathematics & Number Theory
- [`Sieve_of_Eratosthenes.cpp`](./Sieve_of_Eratosthenes.cpp) - Prime Number Generation
- [`Function_IsPrime.cpp`](./Function_IsPrime.cpp) - Fast Primality Test
- [`Binary_Exponentiation.cpp`](./Binary_Exponentiation.cpp) - Calculates \(a^n\) in \(O(\log n)\)
- [`Whell_Factorization.cpp`](./Whell_Factorization.cpp) - Wheel Factorization
- [`Sum_of_Divisiors.cpp`](./Sum_of_Divisiors.cpp) - Calculate the sum of all divisors of a number

### 🔍 Searching, Sorting & Arrays
- [`Binary_Search.cpp`](./Binary_Search.cpp) - Binary Search Implementation
- [`Buble_Sort.cpp`](./Buble_Sort.cpp) - Bubble Sort Algorithm
- [`Kadane_Algorithm.cpp`](./Kadane_Algorithm.cpp) - Maximum Subarray Sum

### 🧠 Dynamic Programming / Techniques
- [`Bitmask.cpp`](./Bitmask.cpp) - Bitmask Operations for DP

## 💡 How to Use
Feel free to clone the repository, explore the snippets, and use these templates for your own CP solutions seamlessly. The implementations are designed to be standalone, straightforward, and easily copy-pasteable.

```bash
git clone https://github.com/Hekimovraiz/Algorithms.git
```

## 🤝 Contribution
If you have improvements or additional algorithms, contributions are always welcome! Let's build the ultimate codebase for competitive programmers!
